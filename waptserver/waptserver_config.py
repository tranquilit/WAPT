...
def load_config(cfgfile=_default_config_file):
    ...
    if os.path.exists(cfgfile):
        ...
        raise Exception("FATAL : couldn't open configuration file : {}.".format(options.configfile))

    if not _config.has_section('options'):
        raise Exception("FATAL, configuration file {} has no section [options]. Please check the waptserver documentation.").format(options.configfile)
