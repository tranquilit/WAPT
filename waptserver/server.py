...
def upload_packages():
    ...
    try:
        ...
        if repositories and app.conf.get('remote_repo_support'):
            repositories.update_file_tree_of_files(os.path.join(os.path.abspath(os.path.join(app.conf['wapt_folder'], os.pardir)),'Sync.json'),[os.path.join(app.conf['wapt_folder']),os.path.join(app.conf['wapt_folder'])+'wua'],emit_websocket=True)
