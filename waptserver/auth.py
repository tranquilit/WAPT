    ...
    for method in methods:
        if method == 'session' and session:
            ...
            if auth_module_ad is not None and (app.conf['wapt_user'] != username and
                    auth_module_ad.check_credentials_ad(app.conf, username, password,dn=app.conf.get('wapt_admin_group_dn'),group_name=app.conf.get('wapt_admin_group'))['member_of_group']):
                auth_method = method
                auth_user = username
                auth_date = datetime.datetime.utcnow().isoformat()
