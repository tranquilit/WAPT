...
#    if not check_if_port_opened(dc_name,389):
#       logger.error("couldn't reach AD")
#       return False

    auth_ok = False

    if re.match('^[\w-]+$', username) is None:
       logger.error("invalid characters in login %s " % username)

    bind_username = '%s@%s' % (username, kerberos_realm.upper())

    ldap_filter = 'cn=%s' % username
    attrs = ['memberOf']
    try:
