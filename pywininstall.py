...
def download_exe(url,hash,location):
    download_location = urllib.urlretrieve(url,location)
    if md5(download_location[0]) == hash:
        print('Good MD5 sum for %s' % (url))
        return download_location[0]
    else:
        os.remove(download_location[0])
        raise('Bad MD5 sum for %s' % (url))
