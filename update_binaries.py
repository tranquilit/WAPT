...
msvc = wget('https://download.microsoft.com/download/d/d/9/dd9a82d0-52ef-40db-8dab-795376989c03/vcredist_x86.exe',resume=True,md5='5689d43c3b201dd3810fa3bba4a6476a',cache_dir=binaries_cache)
...
nssm_zip = wget('https://nssm.cc/ci/nssm-2.24-101-g897c7ad.zip',resume=True,md5='63175d3830b8a5cfd254353c4f561e5c',cache_dir=binaries_cache,connect_timeout=60)
...
pgsql_zip = wget('https://get.enterprisedb.com/postgresql/postgresql-9.4.15-1-windows-x64-binaries.zip',resume=True,md5='cdd7c1bfd656430283899417acf4d82f',cache_dir=binaries_cache)
...
msvc2013 = wget('https://download.microsoft.com/download/2/E/6/2E61CFA4-993B-4DD4-91DA-3737CD5CD6E3/vcredist_x64.exe',resume=True,md5='96b61b8e069832e6b809f24ea74567ba',cache_dir=binaries_cache)
...
print('Get NGINX zip')
nginx_zip = wget('https://nginx.org/download/nginx-1.13.5.zip',resume=True,md5='999bf2444d95771a72eb7fd3637c4f13',cache_dir=binaries_cache)
...
for f in mongoexport_files :
    ...
    if not os.path.isdir(os.path.dirname(makepath(wapt_base_dir,'waptserveur','mongodb'))):
        os.makedirs(os.path.dirname(makepath(wapt_base_dir,'waptserveur','mongodb')))
    os.renames(f,new_name)
...
innoextract_zip = wget('http://constexpr.org/innoextract/files/innoextract-1.6-windows.zip',resume=True,md5='e3abf26e436c8f1858e2e06a67a37b60',cache_dir=binaries_cache)
...
zmq_exe = wget('http://miru.hk/archive/ZeroMQ-4.0.4~miru1.0-x86.exe',resume=True,md5='699b63085408cd7bfcde5d3d62077f4e',cache_dir=binaries_cache)
...
print('Get DMIDecode')
dmidecode = wget('https://github.com/tabad/fusioninventory-agent-windows-installer/blob/master/Tools/dmidecode/x86/dmidecode.exe?raw=true',resume=True,md5='3945000726804e836cfff999e3b330ec',cache_dir=binaries_cache)
...
ssl_zip = wget('https://indy.fulgan.com/SSL/openssl-1.0.2l-i386-win32.zip',resume=True,md5='f1901d936f73d57a9efcef9b028e1621',cache_dir=binaries_cache)
...
python_ldap = wget('https://pypi.python.org/packages/55/8b/7e9b4f4f5c3b4c98416b10ba02f682e8e23d34c20fe8e56b9d09f4667e02/python_ldap-2.4.44-cp27-cp27m-win32.whl',resume=True,md5='21db70f804fe06d941a2e36f907358cf',cache_dir=binaries_cache)
