...
class ADO_record (object):
  ...
  def __str__ (self):
    ...
    s.append (repr (self))
    s.append ("{")
    for name, item in list(self.fields.items ()):
...
def query (query_string, **command_properties):
  ...
  #
  for k, v in list(command_properties.items ()):
...
def ad_time_to_datetime (ad_time):
  ...
  ns100 = (hi << 32) + lo
  delta = datetime.timedelta (microseconds=old_div(ns100, 10))
...
class _AD_object (object):
  ...
  def search (self, *args, **kwargs):
    ...
    if kwargs:
      clauses.append (_and (*("%s='%s'" % (k, v) for (k, v) in list(kwargs.items ()))))
...
def AD_object (obj_or_path=None, path=""):
  ...
  try:
    ...
    raise_(Exception, "Problem with path or object %s" % obj_or_path)
