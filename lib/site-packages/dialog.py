...
def _OSErrorHandling():
    try:
        ...
        raise PythonDialogOSError(str(e))
    except IOError as e:
        raise PythonDialogIOError(str(e))
...
try:
    ...
    raise PythonDialogReModuleError(str(e))
...
def _compute_common_args(mapping):
    ...
    """
    args = []
    for option, value in list(mapping.items()):
...
if sys.hexversion >= 0x030200F0:
    import abc
    # Abstract base class
    class BackendVersion(with_metaclass(abc.ABCMeta, object)):
        @abc.abstractmethod
        def __unicode__(self):
...
class Dialog(object):
    ...
    def _call_program(self, cmdargs, **kwargs):
        ...
        for var, value in list(self._lowlevel_exit_codes.items()):
            varname = "DIALOG_" + var
    ...
    def _generic_form(self, widget_name, method_name, text, elements, height=0,
        ...
        if code == self.HELP:
            ...
            if self._help_status_on(kwargs):
                ...
                # 'status'.
                updated_elements = []
                for elt, updated_item in zip(elements, updated_items):
