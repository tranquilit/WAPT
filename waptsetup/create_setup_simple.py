...
def sign_exe(exe_path,p12path,p12password):
    ...
    for attempt in [1, 2, 3]:
        try:
            ...
            setuphelpers.run(r'"%s" sign /f "%s" /p "%s" /t http://timestamp.globalsign.com/scripts/timestamp.dll "%s"' % (SIGNTOOL,p12path,p12password,exe_path),return_stderr=False)
            break
        except subprocess.CalledProcessError as cpe:
            cpe.cmd =  cpe.cmd.replace(p12password, '********')
            cpe.output = cpe.output.replace(p12password, '********')
            print("Got CalledProcessError from subprocess.check_output: %s %s" % (repr(cpe),repr(cpe.output)))
