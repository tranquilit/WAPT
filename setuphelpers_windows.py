...
from waptutils import Version,makepath,isfile,isdir,killtree,CalledProcessErrorOutput,mkdirs,remove_file,currentdate,currentdatetime,ensure_dir
...
def remove_programs_menu_folder(folder):
    ...
    """
    if len(folder):
        remove_tree(makepath(winshell.programs(common=1),folder))
...
def remove_user_programs_menu_folder(folder):
    ...
    """
    if len(folder):
        remove_tree(makepath(winshell.programs(common=0),folder))
